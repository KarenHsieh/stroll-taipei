...
export function toDisplaySchedule(internalSchedule, area, today, currency, timeZone) {
  ...
  const stops = internalSchedule.stops.map((stop) => ({
    ...
    timeText: toDisplayTime(stop.arriveAt, timeZone),
    ...
    costText: formatCost(stop.attraction.avg_cost, currency),
    todayOpenHoursText: formatTodayOpenHours(stop.attraction.open_hours, today, timeZone),
    indoor: stop.attraction.indoor,
    isPast: false,
  }));

  const lastStop = internalSchedule.stops[internalSchedule.stops.length - 1];
  const endText = formatEndTime(lastStop.leaveAt, timeZone);
